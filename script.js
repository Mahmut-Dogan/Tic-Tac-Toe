...
function draw() {
    for (let i = 0; i < fields.length; i++) {
        if (fields[i] == 'circle') {
            document.getElementById(`circle-${i}`).classList.remove('display-none');
            checkForWin();
        }
        if (fields[i] == 'cross') {
            document.getElementById(`cross-${i}`).classList.remove('display-none');
            checkForWin();
        }
    }
}
...
function restart(){
    ...
    for (let i = 1; i <= 8; i++) {
        document.getElementById('line-' + i ).classList.add('display-none');
        
    }

    for (let i = 0; i <= 8; i++) {
        document.getElementById('circle-' + i ).classList.add('display-none');
        document.getElementById('cross-' + i ).classList.add('display-none');
